...
public class EffectDebilitationStartedProcedure {
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		entity.attackEntityFrom(DamageSource.MAGIC, (float) (((LivingEntity) entity).getAttribute(Attributes.MAX_HEALTH).getBaseValue()
				- (((LivingEntity) entity).getAttribute(Attributes.MAX_HEALTH).getBaseValue() * (1 - (((new Object() {
					int check(Entity _entity) {
						if (_entity instanceof LivingEntity) {
							Collection<EffectInstance> effects = ((LivingEntity) _entity).getActivePotionEffects();
							for (EffectInstance effect : effects) {
