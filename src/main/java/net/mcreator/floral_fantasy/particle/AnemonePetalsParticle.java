...
package net.mcreator.floral_fantasy.particle;
...
public class AnemonePetalsParticle {
	public static final BasicParticleType particle = new BasicParticleType(false);

	@SubscribeEvent
	public static void registerParticleType(RegistryEvent.Register<ParticleType<?>> event) {
		event.getRegistry().register(particle.setRegistryName("anemone_petals"));
	}
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		protected CustomParticle(ClientWorld world, double x, double y, double z, double vx, double vy, double vz, IAnimatedSprite spriteSet) {
			...
			this.maxAge = (int) Math.max(1, 120 + (this.rand.nextInt(20) - 10));
		...
		public void tick() {
			...
			if (!this.isExpired) {
				this.setSprite(this.spriteSet.get((this.age / 30) % 4 + 1, 4));
			}
		}
	}
